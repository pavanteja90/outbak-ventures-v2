...
export class InvestmentManagementComponent implements OnInit {
    ...
    constructor(private router: Router, private googleAnalytics: GoogleAnalyticsService, private meta: Meta,private title: Title) {
        ...
        this.title.setTitle('Investment Management | Services');
        this.meta.updateTag({ name: 'description', content: 'Capital optimisation and management to maximise your returns.' });
        this.meta.updateTag({ name: 'keywords', content: 'investment, investors, management, outbak, ventures' });
    }

    ngOnInit() {
        this.googleAnalytics.emitEvent('PageView', 'Investment Management');
    }
